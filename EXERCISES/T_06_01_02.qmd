...
marriage_df <- marriage_df %>%
  rename("marriage" = "2005")
colnames(marriage_df)
```
...
```{r}
joined <- gapminder %>% 
  inner_join(x = .,  y = marriage_df, by = "country")
glimpse(joined)
```
...
```{r}
joined %>% ggplot() +
geom_smooth(mapping = aes(x = marriage , y = gdpPercap), method = "lm")
```

```{r}
joined %>% ggplot(mapping = aes(y = marriage , x = gdpPercap, color = continent)) +
...
```{r}
joined %>% ggplot(mapping = aes(y = marriage , x = lifeExp, color = continent)) +
geom_smooth( method = "lm") +
  geom_point()
```
