...
mypath <- "datasets_ATRIUM/gapminder_hourly_labour_cost_constant_2017_usd--by--geo--time.csv"
...
```{r}
#| name: readcsv
read_csv(file = mypath, n_max = 3) #just top 3 rows
```
...
```{r}
#| name: savefiles_more
read_csv(file = mypath, col_names = c("country", "year", "USD_hour_2017"), n_max = 3)
```
...
#| name: readrURL
URL <- "https://raw.githubusercontent.com/open-numbers/ddf--gapminder--systema_globalis/refs/heads/master/countries-etc-datapoints/ddf--datapoints--hourly_labour_cost_constant_2017_usd--by--geo--time.csv"
read_csv(file = URL, n_max = 3)
```

## Download an Excel file
```{r}
#| name: downloadexcel
download.file(url = "https://docs.google.com/spreadsheets/d/1qHalit8sXC0R8oVXibc2wa2gY7bkwGzOybEMTWp-08o/export?format=xlsx", 
              destfile = "datasets_ATRIUM/gapminder_geonames.xlsx", mode = "wb")
```
...
#| name: readxlfile
library(readxl)
read_xlsx(path = "datasets_ATRIUM/gapminder_geonames.xlsx", n_max = 3)
...
```{r}
#| name: xlsheets
readxl::excel_sheets("datasets_ATRIUM/gapminder_geonames.xlsx")
```

```{r}
#| name: readxlsheet
readxl::read_xlsx("datasets_ATRIUM/gapminder_geonames.xlsx", sheet = 2, n_max = 3)
# or sheet = "list-of-countries-etc"
```
...
googlesheets4::read_sheet("https://docs.google.com/spreadsheets/d/1qHalit8sXC0R8oVXibc2wa2gY7bkwGzOybEMTWp-08o/edit?gid=425865495#gid=425865495", sheet = 2, n_max = 3)
...
```{r}
#| name: savefiles
gapminder_countries <- readxl::read_xlsx("datasets_ATRIUM/gapminder_geonames.xlsx", sheet = 2, n_max = 3)
readr::write_tsv(gapminder_countries, "my_output_files/gapminder_countries.tsv")
```
