...
- Hazard **loud guesses**. 
  - Often legit assumptions stumble on idiosyncrasy. 
...
![](images_ATRIUM/tidyverse_icons.png)

::: notes
In our course, we are going to focus on one modern R "dialect" for data manipulation, the `tidyverse`. It is a family of software libraries for convenient work on tables. It has a fabulous library for plotting, too. Here you see icons of the libraries with which we are going to work. Tidyverse is associated with the name *Hadley Wickham*, the initial author, but `tidyverse` became so popular that it has grown to an entire ecosystem of complementary libraries with many authors and contributors. The core idea of `tidyverse` is `tidy data`, which means each table row represents one observation and each column represents one variable.
...
::: notes
The brown cells around the blue one at the bottom left give you an idea what it means to work with tabular data:  

A table is called **data frame** in base R. In `tidyverse`, you will often meet the term **`tibble`**. A `tibble` is a data frame adjusted to the tidyverse packages. You do not have to worry about the difference. You can feed data frames to `tidyverse` functions and they just silently convert it to `tibble`.  
:::
